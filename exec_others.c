...
/*   Updated: 2024/09/08 09:20:01 by fiftyblue        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

char	**combine_cmd_arg_with_path(t_cmd *cmd)
{
	char	**new;
	int		i;
...
void	exec_with_execve(t_cmd *cmd, t_sh *sh)
{
	...
	if (!cmd || !sh)
		return ;
	arg = NULL;
	// full_path = is_executable(cmd->cmd, sh);
	// if (full_path)
	arg = combine_cmd_arg_with_path(cmd);
	printf("Executing: %s\n", arg[0]);
